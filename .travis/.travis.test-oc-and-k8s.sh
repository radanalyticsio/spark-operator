...
BIN=${BIN:-oc}
[ "$CRD" = "1" ] && CR="-cr" || CR=""
[ "$CRD" = "1" ] && KIND="sparkcluster" || KIND="cm"
...
testCreateCluster1() {
  info
  os::cmd::expect_success_and_text "${BIN} create -f $DIR/../examples/cluster$CR.yaml" '"?my-spark-cluster"? created' && \
...
testScaleCluster() {
  info
  [ "$CRD" = "1" ] && CMD='sparkcluster my-spark-cluster -p "{\"spec\":{\"workerNodes: 1\"}"' || CMD='cm my-spark-cluster -p "{\"data\":{\"config\": \"workerNodes: 1\"}}"'
  os::cmd::expect_success_and_text '${BIN} patch ${CMD}' '"?my-spark-cluster"? patched' && \
...
testCustomCluster2() {
  ...
  sleep 2
  [ "$CRD" = "1" ] && FOO='cr/' || FOO=''
  os::cmd::expect_success_and_text "${BIN} create -f $DIR/../examples/test/${FOO}cluster-2.yaml" '"?my-spark-cluster-2"? created' && \
...
testCustomCluster3() {
  ...
  sleep 2
  [ "$CRD" = "1" ] && FOO='cr/' || FOO=''
  os::cmd::expect_success_and_text "${BIN} create -f $DIR/../examples/test/${FOO}cluster-with-config-1.yaml" '"?sparky-cluster-1"? created' && \
...
testCustomCluster4() {
  ...
  sleep 2
  [ "$CRD" = "1" ] && FOO='cr/' || FOO=''
  os::cmd::expect_success_and_text "${BIN} create -f $DIR/../examples/test/${FOO}cluster-with-config-2.yaml" '"?sparky-cluster-2"? created' && \
...
testCustomCluster5() {
  ...
  sleep 2
  [ "$CRD" = "1" ] && FOO='cr/' || FOO=''
  os::cmd::expect_success_and_text "${BIN} create -f $DIR/../examples/test/${FOO}cluster-with-config-3.yaml" '"?sparky-cluster-3"? created' && \
